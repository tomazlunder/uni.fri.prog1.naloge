...
#Obvezna naloga
x_vn= float(input("Vnesi koordinato x: "))
y_vn = float(input("Vnesi koordinato y: "))

razdalje = []
for ime, x, y in kraji:
    razdalja = sqrt((x-x_vn)**2+(y-y_vn)**2)
    razdalje += [(razdalja, ime)]

najdlje = max(razdalje)
print(najdlje[1])
...
for ime, x, y in kraji:
    if prva == True:
        ...
        x_1 = x
        y_1 = y
        razdalja1 = sqrt((x_1-x_p)**2+(y_1-y_p)**2)
        vsota = vsota + razdalja1
        x_p = x_1
        y_p = y_1
print(vsota)
