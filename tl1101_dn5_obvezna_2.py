...
def iz_sekund(s):
    ...
    min = (s%3600)//60
    sec = ((s%3600)%60)
    return str(ure)+":"+str(min)+":"+str(sec)

def podatki(s):
    list = s.split("\t")
    return(list[2], int(list[3]), v_sekunde(list[5]), v_sekunde(list[6]))
#Obvezni del
def pospesek(vrstica):
    list = vrstica.split("\t")
    return ((v_sekunde(list[6]))-(v_sekunde(list[5])))/(v_sekunde(list[5]))

def naj_pospesek(vrstice):
    najp = 5000
    reallist =  vrstice.readlines()
    for vrstica in reallist:
        list = vrstica.split("\t")
        c = pospesek(vrstica)
        if najp > pospesek(vrstica):
            ime = list[2]
            najime=ime
            najp = pospesek(vrstica)
    return najime

def vsi_pospeseni(vrstice, faktor):
    list = []
    for vrstica in vrstice:
        if pospesek(vrstica)<= faktor:
            list1 = vrstica.split("\t")
            ime = list1[2]
            list.append(ime)
    return list
...
def tekaci_leta(vrstice, leto):
    ...
    for i in range(len(sez)):
        if len(sez) < clen and len(sez) != 1:
            s = s + ", "
        if len(sez) == 1 and len(sez) != clen:
            s = s + " in "
        s = s + sez[0]
        del sez[0]
    return s
