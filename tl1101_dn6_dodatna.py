...
def barvanje_eno(barve):
    slovar = collections.defaultdict(list)
    for polje, barva in barve:
        if polje not in slovar:
            slovar[polje] = barva
    return slovar
#Obvezne
def prestej_barvo(sahovnica, barva):
    counter = 0
    for polje in sahovnica:
        if sahovnica[polje] == barva:
            counter +=1
    return counter
...
def polja_po_barvah(sahovnica):
    nov_slovar = collections.defaultdict(list)
    for polje in sahovnica:
        if len(nov_slovar[sahovnica[polje]]) == 0:
            nov_slovar[sahovnica[polje]] = set([polje])
        else:
            nov_slovar[sahovnica[polje]].add(polje)
    return nov_slovar

def prestej_barve(sahovnica):
    nov_slovar = collections.defaultdict(list)
    barve_polja = polja_po_barvah(sahovnica)
    for barva in barve_polja:
        nov_slovar[barva] = len(barve_polja[barva])
    return nov_slovar
